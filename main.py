...
def get_most_recent_year_identifiers():           
    ...
    identifiers_to_import = []
    for item in datasets:
        if item['issued'][:4] == most_recent_year:
            identifiers_to_import += item.get('distribution')
            
    return identifiers_to_import

def import_most_recent_year_data(identifiers_to_import):
    # download files from distributions
    for index, item in enumerate(identifiers_to_import):
